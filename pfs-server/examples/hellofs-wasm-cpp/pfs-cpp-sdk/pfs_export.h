...
#define PFS_EXPORT_PLUGIN(PluginType) \
    ...
    char* plugin_name() { \
        ...
        return pfs::ffi::copy_string(g_plugin_instance->name()); \
    } \
    \
    __attribute__((export_name("plugin_get_readme"))) \
    char* plugin_get_readme() { \
        if (!g_plugin_instance) return nullptr; \
        return pfs::ffi::copy_string(g_plugin_instance->readme()); \
    } \
    \
    __attribute__((export_name("plugin_validate"))) \
    char* plugin_validate(const char* config_ptr) { \
        if (!g_plugin_instance) return pfs::ffi::copy_string("not initialized"); \
        pfs::Config config = pfs::ffi::JsonParser::parse_config(config_ptr); \
        auto result = g_plugin_instance->validate(config); \
        if (result.is_err()) { \
            return pfs::ffi::copy_string(result.unwrap_err().to_string()); \
        } \
        return nullptr; \
    } \
    \
    ...
    uint64_t fs_read(const char* path_ptr, int64_t offset, int64_t size) { \
        ...
        std::string path = pfs::ffi::read_string(path_ptr); \
        ...
        uint8_t* buf = (uint8_t*)pfs::ffi::wasm_malloc(len); \
        std::memcpy(buf, data.data(), len); \
        return pfs::ffi::pack_u64((uint32_t)buf, len); \
    } \
    \
    __attribute__((export_name("fs_stat"))) \
    uint64_t fs_stat(const char* path_ptr) { \
        if (!g_plugin_instance) return pfs::ffi::pack_u64(0, (uint32_t)pfs::ffi::copy_string("not initialized")); \
        std::string path = pfs::ffi::read_string(path_ptr); \
        auto result = g_plugin_instance->stat(path); \
        if (result.is_err()) { \
            char* err_ptr = pfs::ffi::copy_string(result.unwrap_err().to_string()); \
            return pfs::ffi::pack_u64(0, (uint32_t)err_ptr); \
        } \
        std::string json = pfs::ffi::JsonParser::serialize_fileinfo(result.unwrap()); \
        char* json_ptr = pfs::ffi::copy_string(json); \
        return pfs::ffi::pack_u64((uint32_t)json_ptr, 0); \
    } \
    \
    ...
    uint64_t fs_readdir(const char* path_ptr) { \
        ...
        std::string json = pfs::ffi::JsonParser::serialize_fileinfo_array(result.unwrap()); \
        char* json_ptr = pfs::ffi::copy_string(json); \
        return pfs::ffi::pack_u64((uint32_t)json_ptr, 0); \
    } \
    \
    ...
    char* fs_rename(const char* old_path_ptr, const char* new_path_ptr) { \
        if (!g_plugin_instance) return pfs::ffi::copy_string("not initialized"); \
        std::string old_path = pfs::ffi::read_string(old_path_ptr); \
        std::string new_path = pfs::ffi::read_string(new_path_ptr); \

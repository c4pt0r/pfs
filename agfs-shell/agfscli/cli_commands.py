...
def cmd_upload(client, local_path: str, agfs_path: str, recursive: bool = False):
    ...
    if os.path.isdir(local_path):
        ...
        try:
            stat_info = client.stat(agfs_path)
            if stat_info.get('isDir'):
                # Remote path exists and is a directory
                # Create subdirectory with same name as local directory
                agfs_path = f"{agfs_path.rstrip('/')}/{local_dir_name}"
                console.print(f"Target is a directory, uploading to {agfs_path}", highlight=False)
        except Exception:
            # Remote path doesn't exist, that's fine
            pass

        _upload_directory(client, local_path, agfs_path)
    else:
        # Upload single file
        ...
        try:
            ...
            if stat_info.get('isDir'):
                ...
                agfs_path = f"{agfs_path.rstrip('/')}/{local_filename}"
                console.print(f"Target is a directory, uploading to {agfs_path}", highlight=False)
        except Exception:
            # Remote path doesn't exist or stat failed, use as-is
            pass

        _upload_file(client, local_path, agfs_path)

def _upload_file(client, local_path: str, agfs_path: str):
    ...
    try:
        ...
        # Upload to AGFS
        msg = client.write(agfs_path, content)

        # Show success message with file size
        size = len(content)
        if msg:
            print(f"Uploaded {local_path} -> {agfs_path} ({size} bytes): {msg}")
        else:
            print(f"Uploaded {local_path} -> {agfs_path} ({size} bytes)")
    except Exception as e:
        console.print(f"upload: {local_path}: {e}", highlight=False)


def _upload_directory(client, local_dir: str, agfs_dir: str):
    """Upload a directory recursively"""
    # Create the destination directory
    try:
        client.mkdir(agfs_dir)
        console.print(f"Created directory {agfs_dir}", highlight=False)
    except Exception as e:
    ...
    for root, dirs, files in os.walk(local_dir):
        ...
        # Calculate AGFS path
        if rel_path == '.':
            current_agfs_dir = agfs_dir
        else:
            # Convert Windows paths to Unix-style
            rel_path = rel_path.replace('\\', '/')
            current_agfs_dir = f"{agfs_dir}/{rel_path}"

        # Create subdirectories
        for dir_name in dirs:
            subdir_agfs_path = f"{current_agfs_dir}/{dir_name}"
            try:
                client.mkdir(subdir_agfs_path)
                console.print(f"Created directory {subdir_agfs_path}", highlight=False)
            except Exception as e:
                # Directory might already exist, continue
                pass
        ...
        for file_name in files:
            ...
            agfs_file_path = f"{current_agfs_dir}/{file_name}"

            try:
                with open(local_file_path, 'rb') as f:
                    content = f.read()

                msg = client.write(agfs_file_path, content)
                size = len(content)
                if msg:
                    print(f"  {local_file_path} -> {agfs_file_path} ({size} bytes): {msg}")
                else:
                    print(f"  {local_file_path} -> {agfs_file_path} ({size} bytes)")
...
def cmd_download(client, agfs_path: str, local_path: str, recursive: bool = False):
    """Download file or directory from AGFS to local filesystem"""
    # Get info about the AGFS path
    try:
        stat_info = client.stat(agfs_path)
    except Exception as e:
        console.print(f"download: {agfs_path}: {e}", highlight=False)
        return

    if stat_info.get('isDir'):
        # Download directory
        if not recursive:
            console.print(f"download: {agfs_path}: Is a directory (use -r for recursive download)", highlight=False)
            return

        # Get the AGFS directory name
        agfs_dir_name = os.path.basename(agfs_path.rstrip('/'))

        # Check if local path exists and is a directory
        if os.path.exists(local_path):
            if os.path.isdir(local_path):
                # Local path exists and is a directory
                # Create subdirectory with same name as AGFS directory
                local_path = os.path.join(local_path, agfs_dir_name)
        ...
        _download_directory(client, agfs_path, local_path)
    else:
        # Download single file
        # If local_path is a directory, use the AGFS filename
        if os.path.isdir(local_path):
            agfs_filename = os.path.basename(agfs_path)
            local_path = os.path.join(local_path, agfs_filename)
            # Find a unique filename if it already exists
            local_path = _get_unique_filename(local_path)

        _download_file(client, agfs_path, local_path)


def _download_file(client, agfs_path: str, local_path: str):
    """Download a single file"""
    try:
        # Download from AGFS
        content = client.cat(agfs_path)
        ...
        # Show success message with file size
        size = len(content)
        print(f"Downloaded {agfs_path} -> {local_path} ({size} bytes)")
    except Exception as e:
        console.print(f"download: {agfs_path}: {e}", highlight=False)


def _download_directory(client, agfs_dir: str, local_dir: str):
    ...
    queue = deque([(agfs_dir, local_dir)])

    while queue:
        current_agfs_dir, current_local_dir = queue.popleft()

        # List directory contents
        try:
            files = client.ls(current_agfs_dir)
        except Exception as e:
            console.print(f"download: {current_agfs_dir}: {e}", highlight=False)
            continue
        ...
        for file_info in files:
            ...
            agfs_file_path = f"{current_agfs_dir.rstrip('/')}/{file_name}"
            ...
            if is_dir:
                ...
                try:
                    ...
                    queue.append((agfs_file_path, local_file_path))
                except Exception as e:
                    console.print(f"download: {local_file_path}: {e}", highlight=False)
            else:
                # Download file
                try:
                    content = client.cat(agfs_file_path)

                    with open(local_file_path, 'wb') as f:
                        f.write(content)

                    size = len(content)
                    print(f"  {agfs_file_path} -> {local_file_path} ({size} bytes)")

                    total_files += 1
                    total_bytes += size
                except Exception as e:
                    console.print(f"download: {agfs_file_path}: {e}", highlight=False)

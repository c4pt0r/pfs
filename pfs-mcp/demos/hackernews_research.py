...
"""
        ...
        success = enqueue_task(queue_path, task_prompt, pfs_api_url)
...
def enqueue_task(
    ...
    try:
        ...
        api_url = pfs_api_url or "http://localhost:8080"
...
def wait_for_results(
    ...
    while len(collected_results) < expected_count:
        ...
        result_files = list_files(results_path, pfs_api_url)
...
def list_files(path: str, pfs_api_url: Optional[str] = None) -> List[str]:
    """List files in a PFS directory"""
    try:
        # Initialize PFS client
        api_url = pfs_api_url or "http://localhost:8080"
        client = PFSClient(api_url)
...
def read_file(file_path: str, pfs_api_url: Optional[str] = None) -> Optional[str]:
    """Read a file from PFS"""
    try:
        # Initialize PFS client
        api_url = pfs_api_url or "http://localhost:8080"
        client = PFSClient(api_url)
...
*Generated by PFS Parallel Research System*
"""
